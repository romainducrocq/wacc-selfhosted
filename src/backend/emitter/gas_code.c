...
static void emit(Ctx ctx, char* code) { write_buffer(ctx->fileio, code); }
...
static void emit_identifier(Ctx ctx, TIdentifier identifier) {
    ...
    emit(ctx, "_");
#endif
    char* value = map_get(ctx->identifiers->hash_table, identifier);
    emit(ctx, value);
}

// string -> $ string
static void emit_string(Ctx ctx, TIdentifier string_const) {
    char* value = map_get(ctx->identifiers->hash_table, string_const);
    emit(ctx, value);
}
...
static void emit_dbl(Ctx ctx, TIdentifier dbl_const) {
    char* value = map_get(ctx->identifiers->hash_table, dbl_const);
    emit(ctx, value);
}
...
static void mov_sx_instr(Ctx ctx, AsmMovSx* node) {
    ...
    emit(ctx, " ");
    {
        TInt byte = type_align_bytes(node->asm_type_src);
        emit_op(ctx, node->src, byte);
    }
    emit(ctx, ", ");
    {
        TInt byte = type_align_bytes(node->asm_type_dst);
        emit_op(ctx, node->dst, byte);
    }
    emit(ctx, LF);
}
...
void emit_gas_code(unique_ptr_t(AsmProgram) * asm_ast, BackEndContext* backend, FileIoContext* fileio,
    IdentifierContext* identifiers) {
    GasCodeContext ctx;
    {
        ctx.backend = backend;
